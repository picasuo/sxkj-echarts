...
export default {
  ...
  methods: {
    dataHandler() {
      ...
      let baseOpts = this.chartHandler(
        xkeys,
        ykeys,
        values,
        this.settings,
    ...
    setChartsOption(opts) {
      ...
      options = opts
      setTimeout(() => {
        this.echarts.setOption(opts, { notMerge: true })
      }, 1000) // notMerge 是否和之前的配置合并
      this.echarts.on('dblclick', function(params) {
        if (options.dblclick && typeof options.dblclick === 'function') {
          options.dblclick(params)
        }
      })
    },
